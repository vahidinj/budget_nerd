...
export class ErrorBoundary extends React.Component<React.PropsWithChildren, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, info: React.ErrorInfo) {
    this.setState({ info });
    // Optional: send to telemetry endpoint if desired
    try {
      fetch('/api/telemetry', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ event:'ui_error', meta:{ message: error.message, stack: error.stack?.slice(0,400) } }) });
    } catch {/* ignore */}
  }
      ...
      return (
        <div className="themed-modal-content" style={{ width: 'min(720px, 92%)', maxWidth: '920px' }}>
          ...
          <p style={{ maxWidth:480, lineHeight:1.4, color: 'var(--text-base)' }}>
            The app encountered an unexpected error. Try <span className="kw">Reset View</span>. If the issue continues, <span className="kw">Reload Page</span> or clear local data.
          </p>
          {this.state.error && (
